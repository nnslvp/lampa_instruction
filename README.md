...
- **Мёртвые плагины никогда не удаляются.** Если плагин умер навсегда — ставим в карточке `status: "Не работает"`, `status_icon: "❌"` и в `status_description` указываем альтернативу (образец — `_plugins/videocdn.html`). URL страницы сохраняется: карточка продолжает собирать поисковый трафик и честно сообщает статус; из /working-plugins она выпадает автоматически (фильтр по `status: "Активен"`).
